...
from model import EVA, HearMMEA
...
class Runner:
    ...
    def model_choise(self):
        ...
        if self.args.model_name == "EVA":
            ...
            self.model = HearMMEA(self.KGs, self.args)
    ...
    def run(self):
        ...
        with tqdm(total=self.args.epoch) as _tqdm:  
            for i in range(self.args.epoch):
                ...
                # if self.epoch == self.args.lr_change_epoch:
                #     self.args.lr = self.args.lr / 5
                #     for param_group in self.optimizer.param_groups:
                #         param_group['lr'] = self.args.lr
                #     self.logger.info(f"Epoch {self.epoch}: 学习率更新为 {self.args.lr}")
    ...
    def train(self, _tqdm):
        ...
        for batch in self.train_dataloader:
            ...
            self.logger.info(output['loss_dic'])
