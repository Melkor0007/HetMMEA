...
class cfg():
    def __init__(self):
        ...
        self.data_root = "/root/SNAG_MMEA/data"
    ...
    def get_args(self):
        ...
        parser.add_argument("--model_name", default="EVA", type=str, choices=["EVA", "MCLEA", "MSNEA", "MEAformer", "SNAG"], help="model name")
        ...
        parser.add_argument("--stage", type=int, default=1, help="which stage of the model", choices=[0, 1, 2])
        parser.add_argument("--ablation", type=int, default=-1, help="which stage of the model", choices=[-1, 0, 1, 2, 3])
        parser.add_argument("--adapt", type=str, default="mlp", help="which visual adapt", choices=["mlp", "vae"])
        ...
        parser.add_argument("--stage_epoch", type=str, default="250,0,0")
        ...
        parser.add_argument("--il_stage_epoch", type=str, default="")
        # --------- MSNEA -----------
        ...
        parser.add_argument("--lr_change_epoch",type=int, default=400)
    ...
    def update_train_configs(self):
        ...
        if self.cfg.model_name not in ["MEAformer", "MSNEA", "EVA", "MCLEA", "SNAG"]:
            ...
            if self.cfg.il:
                ...
                if self.cfg.data_choice == "DBP15K":
                    if not self.cfg.use_surface:
                        self.cfg.weight_decay = max(0.001, self.cfg.weight_decay)
                    if self.cfg.model_name == "SNAG" and self.cfg.data_split in ["ja_en", "fr_en"]:
                        self.cfg.epoch = 1000
                        self.cfg.il_start = 500
            else:
                if self.cfg.data_choice == "DBP15K":
                    if self.cfg.model_name == "SNAG" and self.cfg.data_split in ["ja_en", "fr_en"]:
                        self.cfg.epoch = 500
